...
public class MainActivity extends AppCompatActivity {
    ...
    ValueCallback<Uri> mUploadMessage;//用于访问本地存储卡的Uri
...
//        setSupportActionBar(toolbar);
    ...
    private void initWebView() {
        ...
        mWebView.registerHandler("functionOpen",  new BridgeHandler() {
            ...
            public void handler(String data, CallBackFunction function) {
                ...
                pickFile();//打开app的本地文件，就是和app进行交互了
                function.onCallBack("打开文件预览");
        ...
        user.setLocation("上海");
        user.setName("Bruce");
        // 回调 "functionInJs",已经在js中注册了functionInJs了,程序一进来就执行这里
        mWebView.callHandler("functionInJs", new Gson().toJson(user), new CallBackFunction() {//要地址就给他返回一个user对象
            @Override
    ...
    class myHadlerCallBack extends DefaultHandler {
        ...
        public void handler(String data, CallBackFunction function) {
            if(function != null){
                ...
                Toast.makeText(MainActivity.this, data + "myHadlerCallBack》》》》》", Toast.LENGTH_SHORT).show();
                function.onCallBack("这是默认的handler返回给js的数据");
            }
        }
    }

    public void pickFile() {//打开本地存储
    ...
    public void btnClick(View v) {
        mWebView.callHandler("functionInJs", "重新加载", new CallBackFunction() {//要地址就给他返回一个user对象
            @Override
            public void onCallBack(String data) {
                //data为js返回的数据
                Toast.makeText(MainActivity.this, "重新加载，"+ data, Toast.LENGTH_SHORT).show();
